...
export class ActivitybarPart extends Part implements IPaneCompositeSelectorPart {
	...
	override createContentArea(parent: HTMLElement): HTMLElement {
		...
		// Create Logo
		if (!this.productService.configurationExtra?.lightMode || this.configurationService.getValue('workbench.settings.readOnly')) {
			this.createLogo(this.content);
		}
	...
	private createLogo(content: HTMLElement): void {
		const logo = document.createElement('a');
		logo.title = 'Code Studio';

		if (location.href.includes('studio-lw')) {
			logo.href = 'https://studio.intra.xiaojukeji.com';
			logo.target = '_blank';
		} else {
			logo.href = '/';
		}
	...
	private shouldBeHidden(viewContainerOrId: string | ViewContainer, cachedViewContainer?: ICachedViewContainer): boolean {
		...
		if (this.productService.configurationExtra?.lightMode && viewContainerId === 'workbench.view.debug') {
			return true;
		}

		if (this.productService.configurationExtra?.lightMode && viewContainerId === 'workbench.view.extensions') {
			return true;
		}
